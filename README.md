...
> 派生版本说明：本仓库基于 [Rimagination/dy-note](https://github.com/Rimagination/dy-note) 的 MIT 许可代码继续开发，保留原许可证与来源说明。本版本新增来源绑定、敏感 URL 递归清理、评论 CSV 安全处理、抓取完整性标记和自动化自测；它不是上游项目的官方发行版。
...
- `assets/comments/comments.full.json` / `comments.full.csv`：显式全量抓取且未触及页数上限后的完整可见评论备份；抓取被截断时会自动降级为 sample。
...
默认会复用同一来源的已有产物。输出目录与作品 ID、来源 URL 或本地输入文件指纹绑定；检测到来源变化时会停止，而不是静默复用旧材料。
...
如果换了视频链接、分享文案或本地输入文件，请使用新的 `--out-dir`。`--force` 不允许把已绑定目录改作另一个来源，以免旧媒体、字幕和资产包混入新结果。
...
本项目使用 MIT License，详见 `LICENSE`。派生来源和本版本修改范围见 `NOTICE`；安全边界和漏洞报告方式见 `SECURITY.md`。
